    ...
    <div className='flex items-center justify-between w-full h-[55px] rounded-xl mx-auto py-2 bg-bgPrimary' style={{ border: `1px solid rgba(97, 102, 241, 0.16)` }}>
      <div className="flex items-center gap-2 sm:gap-5 mx-4  w-[130px] h-[46px]">
        ...
        <div className="flex flex-col items-start justify-center gap-[2.63px] w-12 h-10 sm:h-10 sm:w-[4.8rem]">
          <p className='w-[280px] font-medium text-start para text-[12px] sm:text-[16px] text-textPrimary sm:w-max sm:h-[21px]'>{windowWidth<600? nameShortner(name,20) : nameShortner(name,70)}</p>
          <p className='text-textPrimary text-start text-[10px] sm:text-[12px] sm:w-[271px] sm:h-[16px] para'>{windowWidth<700? nameShortner(getOrigin(url),15): nameShortner(getOrigin(url),30)}</p>
        </div>
      </div>
