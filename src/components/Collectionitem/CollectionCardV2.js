...
const CollectionitemV2 = ({
  ...
  return (
    <>
      <div className="relative bg-bgPrimary border  border-neutral-300 rounded-lg w-[48%] md:w-[calc(100%/2-24px)] lg:w-[calc(100%/3-24px)] xl:w-[calc(100%/4-24px)] 3xl:w-[calc(100%/5-20px)] group 
        hover:shadow-md h-[210px] transition duration-300 ease-in-out">
        {
          isOwner && (
            <div className="absolute p-1 transition-all duration-500 rounded-sm opacity-0 cursor-pointer bg-black/[0.20] top-2 left-2 group-hover:opacity-100"
              onClick={
                () => {
        ...
        <Link to={`/${username}/c/${id}`}>
          ...
          <img src={image !== "undefined" && image !== undefined ? image : defultCollectionImage} className="object-cover w-full h-full rounded-t-md " alt="collection img"
          />
          </div>
